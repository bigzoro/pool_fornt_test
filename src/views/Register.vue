...
const doSubmitRegister = async (verifyData) => {
  data.value.key = verifyData.key
  data.value.dots = verifyData.dots
  data.value.x = verifyData.x
  data.value.register_way = activeName.value
  try{
    let result = await registerRequest(data.value)
    console.log("注冊結果：", result)
  }catch(err){
    console.log('注册错误：', err)
  }
  // httpPost('/api/v1/user/register', data.value).then((res) => {
